...
class MyNotes {
  ...
  deleteNote(e) {
    // this saves a variable that targets what li element or (note) was clicked on
  ...
  updateNote(e) {
    ...
    $.ajax({
      ...
      success: (response) => {
        this.makeNoteReadOnly(thisNote);
        console.log('Congrats its been deleted');
        console.log(response);
      },
      error: (err) => {
        console.log('Sorry its not been deleted');
        console.log(response);
      },
  ...
  createNote(e) {
    ...
    // setting the status to private will be the first step to making user content private but its not enough.
    const ourNewPost = {
